...
// Memória de rolagem: guarda a posição do scroll antes de "entrar" numa tela
// (lista → detalhe) e restaura ao "voltar", pra navegação parecer contínua
// em vez de sempre pular pro topo.
const RF_SCROLL_STACK = [];
function pushScroll() { RF_SCROLL_STACK.push(window.scrollY || 0); }
function popScroll() {
    const y = RF_SCROLL_STACK.pop();
...
function buildTabelaPeriodos(ativos, refTaxas, today, modo) {
    const rows = [];
    const end = new Date(today + 'T00:00:00');
    const inicioCarteira = periodStart('todo', today, ativos);
    if (modo === 'mes') {
        const mesesDesdeInicio = Math.max(1, Math.round(diffDays(inicioCarteira, today) / 30) + 1);
        const n = Math.min(24, mesesDesdeInicio);
        for (let i = 0; i < n; i++) {
            const d = new Date(end.getFullYear(), end.getMonth() - i, 1);
            const inicio = d.toISOString().slice(0, 10);
            if (inicio < inicioCarteira && d.getFullYear() === end.getFullYear() - Math.floor((n) / 12) - 1)
                continue;
            const last = new Date(d.getFullYear(), d.getMonth() + 1, 0);
            const fim = last > end ? today : last.toISOString().slice(0, 10);
            if (inicio >= fim || fim < inicioCarteira)
                continue;
            const ret = periodReturnMD(ativos, inicio, fim, refTaxas, today, {});
            if (ret == null)
                continue;
            const cdi = cdiReturnMD(ativos, inicio, fim, refTaxas, today);
            const ganho = periodGainBRL(ativos, inicio, fim, refTaxas, today);
            rows.push({ key: inicio, label: `${MESES_ABREV[d.getMonth()]}/${String(d.getFullYear()).slice(2)}`, ret, cdi, ganho });
        }
    }
    else {
        const anoAtual = end.getFullYear(), anoInicio = Number(inicioCarteira.slice(0, 4));
        for (let y = anoAtual; y >= anoInicio; y--) {
            const inicio = y === anoInicio ? inicioCarteira : `${y}-01-01`;
            const fim = y === anoAtual ? today : `${y}-12-31`;
            if (inicio >= fim)
                continue;
            const ret = periodReturnMD(ativos, inicio, fim, refTaxas, today, {});
            if (ret == null)
            ...
            const cdi = cdiReturnMD(ativos, inicio, fim, refTaxas, today);
            const ganho = periodGainBRL(ativos, inicio, fim, refTaxas, today);
            rows.push({ key: inicio, label: String(y), ret, cdi, ganho });
        }
    }
    return rows;
}
function TabelaPeriodica({ ativos, refTaxas, today }) {
    const [modo, setModo] = useState('mes');
    const rows = buildTabelaPeriodos(ativos, refTaxas, today, modo);
    if (!rows.length)
        return null;
    return React.createElement(RefCard, null,
        React.createElement("div", { className: "section-head" },
            React.createElement("div", null,
                React.createElement("h2", null, "Resultado financeiro e rentabilidade"),
                React.createElement("p", null, "Resultado da carteira e do CDI por per\u00EDodo")),
            React.createElement("div", { className: "chips compact" }, [['mes', 'Por m\u00EAs'], ['ano', 'Por ano']].map(x => React.createElement("button", { key: x[0], className: modo === x[0] ? 'on' : '', onClick: () => setModo(x[0]) }, x[1])))),
        React.createElement("div", { className: "periodo-table" },
            React.createElement("div", { className: "periodo-head" },
                React.createElement("span", null, modo === 'mes' ? 'M\u00EAs' : 'Ano'),
                React.createElement("span", null, "Resultado"),
                React.createElement("span", null, "Carteira"),
                React.createElement("span", null, "CDI")),
            rows.map(r => React.createElement("div", { className: "periodo-row", key: r.key },
                React.createElement("span", null, r.label),
                React.createElement("span", { className: r.ganho >= 0 ? 'green-txt' : 'warn' }, fmtBRL(r.ganho)),
                React.createElement("span", { className: r.ret >= 0 ? 'green-txt' : 'warn' }, fmtPct(r.ret)),
                React.createElement("span", { className: "muted" }, r.cdi == null ? '\u2014' : fmtPct(r.cdi))))));
}
...
// Rendimento líquido de UM investimento num período curto (ex.: mês atual),
// já líquido de IR (na alíquota vigente da posição). Diferente da rentabilidade
// "desde a aplicação" (que é uma média histórica e pode divergir entre duas
// aplicações de mesma taxa abertas em datas diferentes, por causa de mudanças
// no CDI ao longo do tempo): isso mede só o que rendeu AGORA, no período — por
// isso duas posições na mesma taxa e mesma faixa de IR dão o mesmo número.
function rendimentoPeriodoLiquido(inv, m, inicio, fim, ref, today) {
    if (!inicio || !fim || inicio >= fim)
        return null;
    const partida = inv.dataAplicacao > inicio ? inv.dataAplicacao : inicio;
    if (partida >= fim)
        return null;
    const vIni = valorProjetadoEm(inv, partida, ref, today);
    const vFim = valorProjetadoEm(inv, fim, ref, today);
    if (!(vIni > 0))
        return null;
    const retBruto = (vFim / vIni - 1) * 100;
    const fatorLiquido = inv.isentoIR ? 1 : (1 - (m.aliqIR || 0) / 100);
    return retBruto * fatorLiquido;
}
...
function dadosParciais(refTaxas, today) {
    // O Bacen publica o CDI/Selic do dia com defasagem de 1 dia útil: o valor
    // de hoje só sai amanhã de manhã. Enquanto isso, comparações de período que
    // incluem "hoje" ficam paradas no último dado confirmado (não é bug).
    return !!(refTaxas.dataCDI && refTaxas.dataCDI < today);
}
...
function ReferenceDashboard({ ativos, totais, ganhoLiquido, metricsById, refTaxas, today, evolucao, setTab, period, setPeriod, diasAlerta, onNew }) {
    ...
    const vencProximos = diasAlerta > 0 ? ativos.filter(i => metricsById[i.id].diasRestantes != null && metricsById[i.id].diasRestantes >= 0 && metricsById[i.id].diasRestantes <= diasAlerta) : [];
    return React.createElement("div", { className: "screen" },
        React.createElement("div", { className: "page-head" },
            React.createElement("h1", null, "Painel"),
            React.createElement("button", { className: "circle-plus", onClick: onNew, "aria-label": "Nova aplica\u00E7\u00E3o" },
        ...
        dadosParciais(refTaxas, today) && React.createElement("div", { className: "partial-note" },
            React.createElement(Icon, { name: "info", size: 13 }),
            React.createElement("span", null,
                "O Bacen ainda n\u00E3o publicou o CDI de hoje (",
                fmtData(today),
                ") \u2014 valores consideram at\u00E9 ",
                fmtData(refTaxas.dataCDI),
                ". Atualiza sozinho amanh\u00E3.")),
            ...
            React.createElement(RefCard, { className: "clickable atencao-card", onClick: () => setTab('aplicacoes') },
                ...
                React.createElement("div", { className: "atencao-row" },
                    ...
                    React.createElement("div", null,
                        ...
                        React.createElement("span", null, diasAlerta > 0 ? `vencimentos em at\u00E9 ${diasAlerta} dias` : 'alertas desativados'))))));
}
function ReferenceApplications({ ativos, metricsById, refTaxas, today, setTab, onNew, openEdit, deleteInvestment, onResgatar, pendingInvId, clearPending, voltarAoTab, clearVoltar }) {
    const [filter, setFilter] = useState('Todas'), [q, setQ] = useState(''), [searchOpen, setSearchOpen] = useState(false), [selected, setSelected] = useState(null), [selectedPos, setSelectedPos] = useState(null), [selectedInv, setSelectedInv] = useState(null), [voltouDe, setVoltouDe] = useState(null);
    useEffect(() => {
        if (!pendingInvId)
            return;
    ...
    const backToList = () => { popScroll(); setSelected(null); setSelectedPos(null); setSelectedInv(null); };
    ...
    if (g && selectedInv) {
        ...
        if (!inv)
            return null;
        return React.createElement("div", { className: "screen" },
            React.createElement("button", { className: "back-btn", onClick: voltarDoDetalhe },
                "\u2039 ",
                voltouDe === 'analise' ? "An\u00E1lise" : selectedPos ? "posi\u00E7\u00E3o" : nomeInstituicao(g.nome)),
    ...
    if (g && selectedPos) {
        ...
        if (!pos)
            return null;
        return React.createElement("div", { className: "screen" },
            React.createElement("button", { className: "back-btn", onClick: () => { popScroll(); setSelectedPos(null); } },
                "\u2039 ",
            ...
            React.createElement("div", { className: "detail-list" }, pos.lots.slice().sort((a, b) => a.dataAplicacao.localeCompare(b.dataAplicacao)).map(inv => {
                ...
                return React.createElement(RefCard, { key: inv.id, className: "investment-card" },
                    React.createElement("button", { className: "investment-hit", onClick: () => { pushScroll(); setSelectedInv(inv.id); } },
    ...
    if (g) {
        ...
        return React.createElement("div", { className: "screen" },
            ...
            React.createElement("div", { className: "detail-list" }, posicoes.map(pos => {
                const card = React.createElement("button", { className: "investment-hit", onClick: () => { pushScroll(); if (pos.lots.length > 1)
                            setSelectedPos(pos.key);
                        else
                            setSelectedInv(pos.lots[0].id); } },
                    React.createElement("div", { className: "inv-top" },
    ...
    return React.createElement("div", { className: "screen" },
        ...
        React.createElement("div", { className: "institution-list" }, grupos.map(gr => {
            ...
            return React.createElement("button", { className: "institution-row", key: gr.key, onClick: () => { pushScroll(); setSelected(gr.key); } },
...
function buildRentabilidadePercentSeries(ativos, refTaxas, today) {
    const inicio = periodStart('todo', today, ativos);
    const start = new Date(inicio + 'T00:00:00'), end = new Date(today + 'T00:00:00');
    const totalMonths = Math.max(1, (end.getFullYear() - start.getFullYear()) * 12 + (end.getMonth() - start.getMonth()));
    const step = Math.max(1, Math.ceil(totalMonths / 14));
    const datas = [inicio];
    for (let m = step; m < totalMonths; m += step) {
        const d = new Date(start.getFullYear(), start.getMonth() + m, start.getDate());
        const iso = d.toISOString().slice(0, 10);
        if (iso > inicio && iso < today)
            datas.push(iso);
    }
    datas.push(today);
    return datas.map(dt => ({
        date: fmtData(dt),
        carteira: dt === inicio ? 0 : periodReturnMD(ativos, inicio, dt, refTaxas, today, {}),
        cdi: dt === inicio ? 0 : cdiReturnMD(ativos, inicio, dt, refTaxas, today),
        ipca: dt === inicio ? 0 : ipcaReturnApprox(inicio, dt, refTaxas),
    }));
}
function RentabilidadePercentChart({ ativos, refTaxas, today }) {
    const serie = buildRentabilidadePercentSeries(ativos, refTaxas, today);
    if (serie.length < 2)
        return null;
    const vals = serie.flatMap(p => [p.carteira, p.cdi, p.ipca]).filter(v => v != null);
    if (!vals.length)
        return null;
    const min = Math.min(0, ...vals), max = Math.max(...vals, 0.01), range = max - min || 1, W = 700, H = 220, p = 12;
    const toPts = (key) => serie.map((pt, i) => [p + i / (serie.length - 1) * (W - p * 2), H - p - ((pt[key] ?? 0) - min) / range * (H - p * 2)]);
    const toPath = (pts) => pts.map((q, i) => (i ? 'L' : 'M') + q[0].toFixed(1) + ',' + q[1].toFixed(1)).join(' ');
    ...
    return React.createElement(RefCard, null,
        React.createElement("div", { className: "section-head" },
            React.createElement("div", null,
                React.createElement("h2", null, "Rentabilidade acumulada (%)"),
                React.createElement("p", null, "Carteira x CDI x IPCA desde o in\u00EDcio"))),
        React.createElement("div", { className: "linechart-body" },
            React.createElement("div", { className: "linechart-yaxis" }, gridVals.map((v, i) => React.createElement("span", { key: i }, fmtPct(v)))),
            React.createElement("svg", { viewBox: `0 0 ${W} ${H}`, preserveAspectRatio: "none" },
                gridYs.map((y, i) => React.createElement("line", { key: i, x1: p, x2: W - p, y1: y, y2: y, stroke: "rgba(120,130,150,0.14)", strokeWidth: "1" })),
                React.createElement("path", { d: toPath(toPts('ipca')), fill: "none", stroke: "#F59E0B", strokeWidth: "2", strokeDasharray: "3 3", vectorEffect: "non-scaling-stroke" }),
                React.createElement("path", { d: toPath(toPts('cdi')), fill: "none", stroke: "#2F80ED", strokeWidth: "2", strokeDasharray: "5 4", vectorEffect: "non-scaling-stroke" }),
                React.createElement("path", { d: toPath(toPts('carteira')), fill: "none", stroke: "#22C55E", strokeWidth: "3", vectorEffect: "non-scaling-stroke" }))),
        React.createElement("div", { className: "chart-axis" },
            React.createElement("span", null, serie[0].date),
            React.createElement("span", null, serie[Math.floor(serie.length / 2)].date),
            React.createElement("span", null, ultimo.date)),
        React.createElement("div", { className: "bar-legend" },
            React.createElement("span", null,
                React.createElement("i", { className: "dot green" }),
                " Carteira ", fmtPct(ultimo.carteira)),
            React.createElement("span", null,
                React.createElement("i", { className: "dot blue" }),
                " CDI ", fmtPct(ultimo.cdi)),
            React.createElement("span", null,
                React.createElement("i", { className: "dot", style: { background: '#F59E0B' } }),
                " IPCA ", fmtPct(ultimo.ipca))));
}
function VencimentosFuturosChart({ ativos, metricsById, today }) {
    const buckets = {};
    ativos.forEach(inv => {
        const m = metricsById[inv.id];
        if (m.diasRestantes == null || m.diasRestantes < 0)
            return;
        const d = new Date(inv.dataVencimento + 'T00:00:00');
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
        if (!buckets[key])
            buckets[key] = { key, label: `${MESES_ABREV[d.getMonth()]}/${String(d.getFullYear()).slice(2)}`, valor: 0 };
        buckets[key].valor += m.valorAtualLiquido;
    });
    const rows = Object.values(buckets).sort((a, b) => a.key.localeCompare(b.key)).slice(0, 12);
    if (!rows.length)
        return null;
    ...
    return React.createElement(RefCard, null,
        React.createElement("div", { className: "section-head" },
            React.createElement("div", null,
                React.createElement("h2", null, "Vencimentos futuros"),
                React.createElement("p", null, "Quanto fica dispon\u00EDvel, m\u00EAs a m\u00EAs"))),
        React.createElement("div", { className: "bars-scroll" },
            React.createElement("div", { className: "venc-grid", style: { minWidth: Math.max(300, rows.length * 52) } }, rows.map(r => React.createElement("div", { className: "venc-col", key: r.key },
                React.createElement("small", { className: "venc-value" }, fmtBRLk(r.valor)),
                React.createElement("div", { className: "venc-bar-wrap" },
                    React.createElement("div", { className: "venc-bar", style: { height: `${Math.max(4, r.valor / max * 110)}px` } })),
                React.createElement("small", null, r.label))))));
}
...
function ReferenceAnalysis({ ativos, metricsById, refTaxas, today, evolucao, setTab, onSelectInv }) {
    ...
    return React.createElement("div", { className: "screen" },
        ...
        dadosParciais(refTaxas, today) && React.createElement("div", { className: "partial-note" },
            React.createElement(Icon, { name: "info", size: 13 }),
            React.createElement("span", null,
                "CDI de hoje ainda n\u00E3o publicado pelo Bacen \u2014 considerando at\u00E9 ",
                fmtData(refTaxas.dataCDI),
                ".")),
        React.createElement("div", { className: "two-col stats-row" },
            React.createElement(RefCard, null,
        ...
        React.createElement(RentabilidadePercentChart, { ativos: ativos, refTaxas: refTaxas, today: today }),
        React.createElement(RefCard, null,
            React.createElement("div", { className: "section-head" },
                React.createElement("div", null,
        ...
        React.createElement(TabelaPeriodica, { ativos: ativos, refTaxas: refTaxas, today: today }),
        React.createElement(RefCard, null,
            React.createElement("div", { className: "section-head" },
                React.createElement("div", null,
        ...
        React.createElement(RankingRendimentoMensal, { ativos: ativos, metricsById: metricsById, onSelectInv: onSelectInv, refTaxas: refTaxas, today: today }),
        React.createElement(VencimentosFuturosChart, { ativos: ativos, metricsById: metricsById, today: today }),
        React.createElement(InsightsList, { ativos: ativos, metricsById: metricsById, refTaxas: refTaxas, today: today })));
}
function RankingRendimentoMensal({ ativos, metricsById, onSelectInv, refTaxas, today }) {
    if (ativos.length < 1)
        return null;
    const inicioMes = today.slice(0, 8) + '01';
    const linhas = ativos.map(inv => { const m = metricsById[inv.id]; return { inv, m, rent: rendimentoPeriodoLiquido(inv, m, inicioMes, today, refTaxas, today) }; }).filter(x => x.rent != null);
    if (!linhas.length)
        return null;
    linhas.sort((a, b) => b.rent - a.rent);
    const max = Math.max(...linhas.map(x => Math.abs(x.rent)), 0.01);
    return React.createElement(RefCard, null,
        React.createElement("div", { className: "section-head" },
            React.createElement("div", null,
                React.createElement("h2", null, "Ranking de rendimento (l\u00EDq./m\u00EAs)"),
                React.createElement("p", null, "J\u00E1 descontando o IR vigente de cada aplica\u00E7\u00E3o \u2014 por isso uma taxa menor pode render mais l\u00EDquido se aplicada h\u00E1 mais tempo"))),
        React.createElement("div", { className: "rank-yield-list" }, linhas.map(({ inv, m, rent }, i) => React.createElement("button", { className: "rank-yield-row", key: inv.id, onClick: () => onSelectInv(inv.id) },
            ...
            React.createElement("div", { className: "rank-yield-mid" },
                React.createElement("div", { className: "rank-yield-top" },
                    ...
                    React.createElement("strong", { className: rent >= 0 ? 'green-txt' : 'warn' }, fmtPct(rent))),
                React.createElement("div", { className: "rank-yield-bar" },
                    React.createElement("div", { className: rent >= 0 ? 'pos' : 'neg', style: { width: Math.max(4, Math.abs(rent) / max * 100) + '%' } })),
                React.createElement("span", { className: "muted" },
                    nomeInstituicao(inv.instituicao),
                    " \u00B7 IR ",
                    inv.isentoIR ? 'isento' : fmtPct(m.aliqIR),
                    " \u00B7 h\u00E1 ",
                    m.diasCorridos,
                    " dias")),
            React.createElement(Icon, { name: "chevronRight", size: 15, color: "var(--muted)" })))));
...
const TEMA_LABELS = { auto: 'Automático', dark: 'Escuro', light: 'Claro' };
const ALERTA_LABELS = { 7: '7 dias antes', 15: '15 dias antes', 30: '30 dias antes', 60: '60 dias antes', 90: '90 dias antes', 0: 'Desativado' };
function MoreScreen({ onDados, onNew, setTab, refTaxas, taxaStatus, onRefresh, onRelatorio, historicoCount, tema, setTema, diasAlerta, setDiasAlerta, showSheet, setShowSheet }) {
    return React.createElement("div", { className: "screen" },
        React.createElement("div", { className: "page-head" },
        ...
        React.createElement(RefCard, { className: "settings-card" },
            ...
            React.createElement("button", { className: "settings-row", onClick: () => setShowSheet('moeda') },
                React.createElement("span", null, "Moeda"),
                React.createElement("span", { className: "settings-val" }, "Real (R$)",
                    React.createElement(Icon, { name: "chevronRight", size: 15 }))),
            React.createElement("button", { className: "settings-row", onClick: () => setShowSheet('tema') },
                React.createElement("span", null, "Tema"),
                React.createElement("span", { className: "settings-val" }, TEMA_LABELS[tema] || 'Autom\u00E1tico',
                    React.createElement(Icon, { name: "chevronRight", size: 15 }))),
            React.createElement("button", { className: "settings-row", onClick: () => setShowSheet('notificacoes') },
                React.createElement("span", null, "Notifica\u00E7\u00F5es"),
                React.createElement("span", { className: "settings-val" }, ALERTA_LABELS[diasAlerta] || 'Vencimentos',
                    React.createElement(Icon, { name: "chevronRight", size: 15 })))),
        React.createElement(RefCard, { className: "settings-card" },
            React.createElement("div", { className: "settings-title" }, "Taxas de refer\u00EAncia"),
            React.createElement("button", { className: "settings-row tall", onClick: () => setShowSheet('cdi') },
                React.createElement("span", null, "CDI"),
        ...
        showSheet === 'moeda' && React.createElement(OptionSheet, { title: "Moeda", subtitle: "Por enquanto o Raz\u00E3o trabalha s\u00F3 com Real \u2014 novas moedas podem entrar em vers\u00F5es futuras.", options: [{ value: 'BRL', label: 'Real (R$)' }], value: "BRL", onSelect: () => { }, onClose: () => setShowSheet(null) }),
        showSheet === 'tema' && React.createElement(OptionSheet, { title: "Tema", subtitle: "\"Autom\u00E1tico\" segue o tema do seu celular.", options: [{ value: 'auto', label: 'Autom\u00E1tico' }, { value: 'dark', label: 'Escuro' }, { value: 'light', label: 'Claro' }], value: tema, onSelect: setTema, onClose: () => setShowSheet(null) }),
        showSheet === 'notificacoes' && React.createElement(OptionSheet, { title: "Notifica\u00E7\u00F5es", subtitle: "Define com quanta anteced\u00EAncia o card \"Aten\u00E7\u00E3o\" do Painel avisa sobre vencimentos pr\u00F3ximos.", options: [7, 15, 30, 60, 90, 0].map(v => ({ value: v, label: ALERTA_LABELS[v] })), value: diasAlerta, onSelect: setDiasAlerta, onClose: () => setShowSheet(null) }),
        showSheet === 'cdi' && React.createElement(OptionSheet, { title: "Taxas de refer\u00EAncia", onClose: () => setShowSheet(null), extra: React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 6 } },
                React.createElement("div", { className: "cdi-info-row" },
                    React.createElement("span", null, "CDI"),
                    React.createElement("b", null, fmtPct(refTaxas.cdi)),
                    React.createElement("small", null, "ref. ", fmtData(refTaxas.dataCDI))),
                React.createElement("div", { className: "cdi-info-row" },
                    React.createElement("span", null, "Selic"),
                    React.createElement("b", null, fmtPct(refTaxas.selic)),
                    React.createElement("small", null, "ref. ", fmtData(refTaxas.dataSelic))),
                React.createElement("div", { className: "cdi-info-row" },
                    React.createElement("span", null, "IPCA (12m)"),
                    React.createElement("b", null, fmtPct(refTaxas.ipca)),
                    React.createElement("small", null, "ref. ", fmtData(refTaxas.dataIPCA))),
                React.createElement("div", { style: { fontSize: 10.5, color: C.muted, lineHeight: 1.5 } }, "Fonte: Banco Central (SGS). ", taxaStatus === 'conectado' ? 'Conectado \u2014 atualiza sozinho todo dia.' : taxaStatus === 'carregando' ? 'Atualizando agora\u2026' : 'Sem conex\u00E3o no momento \u2014 usando o \u00FAltimo valor salvo.'),
                React.createElement("button", { onClick: onRefresh, style: { background: C.blue, color: '#fff', border: 'none', borderRadius: 9, padding: '10px 14px', fontWeight: 750, cursor: 'pointer', marginTop: 4 } }, "Atualizar agora")) }));
}
function App() {
    const [loading, setLoading] = useState(true), [investments, setInvestments] = useState([]), [refTaxas, setRefTaxas] = useState(defaultRef), [taxaStatus, setTaxaStatus] = useState('manual'), [historicoIndices, setHistoricoIndices] = useState({ cdi: [], selic: [], status: 'idle' }), [tab, setTabRaw] = useState('painel'), [period, setPeriod] = useState('mes'), [showForm, setShowForm] = useState(false), [editingId, setEditingId] = useState(null), [form, setForm] = useState(emptyForm), [formError, setFormError] = useState(''), [showDados, setShowDados] = useState(false), [backupMsg, setBackupMsg] = useState(''), [today, setToday] = useState(todayStr()), [historicoTick, setHistoricoTick] = useState(0), [showResgate, setShowResgate] = useState(null), [showRelatorio, setShowRelatorio] = useState(false), [pendingInvId, setPendingInvId] = useState(null), [tema, setTema] = useState(() => localStorage.getItem('rf-tema') || 'auto'), [diasAlerta, setDiasAlerta] = useState(() => Number(localStorage.getItem('rf-dias-alerta')) || 60), [showSheet, setShowSheet] = useState(null), [voltarAoTab, setVoltarAoTab] = useState(null);
    const tabScrollRef = React.useRef({});
    const setTab = useCallback((novaTab) => {
        tabScrollRef.current[tab] = window.scrollY || 0;
        setTabRaw(novaTab);
        const y = tabScrollRef.current[novaTab] || 0;
    ...
    useEffect(() => {
        const aplicar = () => {
            const efetivo = tema === 'auto' ? (window.matchMedia && window.matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark') : tema;
            document.documentElement.setAttribute('data-theme', efetivo);
        };
        aplicar();
        localStorage.setItem('rf-tema', tema);
        if (tema !== 'auto' || !window.matchMedia)
            return;
        const mq = window.matchMedia('(prefers-color-scheme: light)');
        mq.addEventListener('change', aplicar);
        return () => mq.removeEventListener('change', aplicar);
    }, [tema]);
    useEffect(() => { localStorage.setItem('rf-dias-alerta', String(diasAlerta)); }, [diasAlerta]);
    ...
    if (showRelatorio)
        return React.createElement(RelatorioPrint, { ativos: ativos, historico: historico, totais: totais, ganhoLiquido: ganhoLiquido, metricsById: metricsById, refTaxas: ref, today: today, evolucao: evolucao, onClose: () => { popScroll(); setShowRelatorio(false); } });
    return React.createElement("div", { className: "ref-app" },
        ...
        React.createElement("main", null,
            tab === 'painel' && React.createElement(ReferenceDashboard, { ativos: ativos, totais: totais, ganhoLiquido: ganhoLiquido, metricsById: metricsById, refTaxas: ref, today: today, evolucao: evolucao, setTab: setTab, period: period, setPeriod: setPeriod, diasAlerta: diasAlerta, onNew: openNew }),
            " ",
            tab === 'aplicacoes' && React.createElement(ReferenceApplications, { ativos: ativos, metricsById: metricsById, refTaxas: ref, today: today, setTab: setTab, onNew: openNew, openEdit: openEdit, deleteInvestment: deleteInvestment, onResgatar: setShowResgate, pendingInvId: pendingInvId, clearPending: () => setPendingInvId(null), voltarAoTab: voltarAoTab, clearVoltar: () => setVoltarAoTab(null) }),
            " ",
            tab === 'analise' && React.createElement(ReferenceAnalysis, { ativos: ativos, metricsById: metricsById, refTaxas: ref, today: today, evolucao: evolucao, setTab: setTab, onSelectInv: (id) => { setPendingInvId(id); setVoltarAoTab('analise'); setTab('aplicacoes'); } }),
            " ",
            ...
            " ",
            tab === 'mais' && React.createElement(MoreScreen, { onDados: () => { setBackupMsg(''); setShowDados(true); }, onNew: openNew, setTab: setTab, refTaxas: refTaxas, taxaStatus: taxaStatus, onRefresh: refreshTaxas, onRelatorio: () => { pushScroll(); setShowRelatorio(true); }, historicoCount: historico.length, tema: tema, setTema: setTema, diasAlerta: diasAlerta, setDiasAlerta: setDiasAlerta, showSheet: showSheet, setShowSheet: setShowSheet })),
...
function OptionSheet({ title, subtitle, options, value, onSelect, onClose, extra }) {
    return React.createElement("div", { style: { position: 'fixed', inset: 0, background: '#000000C8', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 60 }, onClick: onClose },
        React.createElement("div", { style: { background: C.panel, borderRadius: '18px 18px 0 0', width: '100%', maxWidth: 480, padding: '18px 20px max(18px, env(safe-area-inset-bottom))', boxShadow: shadow, borderTop: `1px solid ${C.hairline}`, maxHeight: '80vh', overflowY: 'auto' }, onClick: e => e.stopPropagation() },
            React.createElement("div", { style: { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: subtitle ? 4 : 14 } },
                React.createElement("h2", { style: { fontSize: 16, fontWeight: 800, margin: 0 } }, title),
            ...
            subtitle && React.createElement("p", { style: { fontSize: 11.5, color: C.muted, lineHeight: 1.5, margin: '0 0 14px' } }, subtitle),
            extra,
            options && React.createElement("div", { style: { display: 'flex', flexDirection: 'column' } }, options.map(opt => {
                const check = opt.value === value ? React.createElement(Icon, { name: "check", size: 16, color: C.blue }) : null;
                return React.createElement("button", { key: String(opt.value), onClick: () => { onSelect(opt.value); onClose(); }, style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'none', border: 'none', borderTop: `1px solid ${C.hairline}`, padding: '13px 2px', color: C.text, textAlign: 'left', cursor: 'pointer', fontSize: 13.5, fontWeight: opt.value === value ? 750 : 500 } },
...
function HistoricoScreen({ historico, refTaxas, today, reabrirInvestimento, deleteInvestment, setTab }) {
    ...
    if (sel) {
        ...
        return React.createElement("div", { className: "screen" },
            React.createElement("button", { className: "back-btn", onClick: () => { popScroll(); setSelectedInv(null); } }, "\u2039 Hist\u00F3rico"),
            React.createElement(RefCard, { className: "detail-hero" },
                React.createElement("div", null,
            ...
            React.createElement(RefCard, { className: "detail-rows" },
                ...
                React.createElement("div", { className: "detail-row" },
                    ...
                    React.createElement("strong", null, taxaCurta(inv)))),
            React.createElement("div", { className: "detail-actions" },
                React.createElement("button", { className: "text-btn", onClick: () => { reabrirInvestimento(inv.id); popScroll(); setSelectedInv(null); } }, "Reabrir como ativo"),
...
const PALETA_RELATORIO = ['#2F80ED', '#F59E0B', '#22C55E', '#8B5CF6', '#EF4444', '#16B7D8', '#EAB308'];
function donutSlicePath(cx, cy, r, r2, a0, a1) {
    const toXY = (ang, radius) => [cx + radius * Math.cos(ang), cy + radius * Math.sin(ang)];
    const [x0, y0] = toXY(a0, r), [x1, y1] = toXY(a1, r);
    const [x2, y2] = toXY(a1, r2), [x3, y3] = toXY(a0, r2);
    const large = (a1 - a0) > Math.PI ? 1 : 0;
    return `M ${x0} ${y0} A ${r} ${r} 0 ${large} 1 ${x1} ${y1} L ${x2} ${y2} A ${r2} ${r2} 0 ${large} 0 ${x3} ${y3} Z`;
}
function RelatorioDonut({ items, total, palette }) {
    let ang = -Math.PI / 2;
    const cx = 70, cy = 70, r = 66, r2 = 40;
    const slices = items.map((it, i) => {
        const frac = total > 0 ? it.valor / total : 0;
        const a0 = ang, a1 = ang + frac * Math.PI * 2;
        ang = a1;
        return { d: donutSlicePath(cx, cy, r, r2, a0, a1), color: palette[i % palette.length] };
    });
    return React.createElement("svg", { width: 140, height: 140, viewBox: "0 0 140 140" }, slices.map((s, i) => React.createElement("path", { key: i, d: s.d, fill: s.color })));
}
function RelatorioLineChart({ evolucao }) {
    if (!evolucao || evolucao.length < 2)
        return null;
    const vals = evolucao.map(x => x.total), min = Math.min(...vals), max = Math.max(...vals), range = max - min || 1, W = 640, H = 130, p = 8;
    ...
    const area = path + ` L${W - p},${H - p} L${p},${H - p} Z`;
    return React.createElement("svg", { viewBox: `0 0 ${W} ${H}`, style: { width: '100%', height: 110, display: 'block' } },
        React.createElement("defs", null,
            React.createElement("linearGradient", { id: "relArea", x1: "0", x2: "0", y1: "0", y2: "1" },
                React.createElement("stop", { offset: "0", stopColor: "#2F80ED", stopOpacity: ".22" }),
                React.createElement("stop", { offset: "1", stopColor: "#2F80ED", stopOpacity: "0" }))),
        React.createElement("path", { d: area, fill: "url(#relArea)" }),
        React.createElement("path", { d: path, fill: "none", stroke: "#2F80ED", strokeWidth: "2.5" }));
}
function RelatorioBars({ rows, maxAbs }) {
    return React.createElement("div", { style: { display: 'flex', alignItems: 'flex-end', gap: 6, height: 90, marginTop: 4 } }, rows.map((r, i) => React.createElement("div", { key: i, style: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 } },
        React.createElement("div", { style: { width: '100%', maxWidth: 22, height: Math.max(3, Math.abs(r.val) / maxAbs * 70), background: r.val >= 0 ? '#22C55E' : '#EF4444', borderRadius: '3px 3px 0 0' } }),
        React.createElement("small", { style: { fontSize: 8, color: '#98A2AF' } }, r.label))));
}
function RelatorioPrint({ ativos, historico, totais, ganhoLiquido, metricsById, refTaxas, today, evolucao, onClose }) {
    ...
    return React.createElement("div", { style: { background: '#F5F6F8', minHeight: '100dvh', color: '#101828', fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" } },
        ...
        React.createElement("div", { style: { maxWidth: 720, margin: '0 auto', padding: '18px 16px 40px' } },
                ...
                React.createElement("div", { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 } },
                    ...
                    React.createElement("div", null,
                        ...
                        React.createElement("div", { style: { fontSize: 15, fontWeight: 800 } },
                            ...
                            grupos.length,
                            " institui\u00E7\u00F5es"))),
                evolucao && evolucao.length > 1 && React.createElement("div", { style: { marginTop: 14 } },
                    React.createElement(RelatorioLineChart, { evolucao: evolucao }))),
            React.createElement("div", { style: section },
            ...
            React.createElement("div", { style: section },
                ...
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap', marginBottom: 12 } },
                    React.createElement(RelatorioDonut, { items: grupos.map(g => ({ valor: g.liquido })), total: totais.liquido, palette: PALETA_RELATORIO }),
                    React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 6, flex: 1, minWidth: 140 } }, grupos.map((g, i) => React.createElement("div", { key: g.key, style: { display: 'flex', alignItems: 'center', gap: 7, fontSize: 11 } },
                        React.createElement("span", { style: { width: 8, height: 8, borderRadius: 2, background: PALETA_RELATORIO[i % PALETA_RELATORIO.length], flexShrink: 0 } }),
                        React.createElement("span", { style: { flex: 1, color: '#344054' } }, nomeInstituicao(g.nome)),
                        React.createElement("b", null, fmtPct(g.liquido / (totais.liquido || 1) * 100)))))),
                React.createElement("table", { style: { width: '100%', borderCollapse: 'collapse' } },
                    React.createElement("thead", null,
                        React.createElement("tr", null,
            ...
            React.createElement("div", { style: section },
                ...
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap', marginBottom: 12 } },
                    React.createElement(RelatorioDonut, { items: tipos.map(t => ({ valor: t.valor })), total: totais.liquido, palette: PALETA_RELATORIO }),
                    React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 6, flex: 1, minWidth: 140 } }, tipos.map((t, i) => React.createElement("div", { key: t.tipo, style: { display: 'flex', alignItems: 'center', gap: 7, fontSize: 11 } },
                        React.createElement("span", { style: { width: 8, height: 8, borderRadius: 2, background: PALETA_RELATORIO[i % PALETA_RELATORIO.length], flexShrink: 0 } }),
                        React.createElement("span", { style: { flex: 1, color: '#344054' } }, t.tipo),
                        React.createElement("b", null, fmtPct(t.valor / (totais.liquido || 1) * 100)))))),
                React.createElement("table", { style: { width: '100%', borderCollapse: 'collapse' } },
                    React.createElement("thead", null,
                        React.createElement("tr", null,
            ...
            ranking.length > 1 && React.createElement("div", { style: section },
                ...
                React.createElement(RelatorioBars, { rows: ranking.slice(0, 10).map(({ inv, m }) => ({ label: nomeInstituicao(inv.instituicao).slice(0, 8), val: m.rentLiquidaMensal })), maxAbs: Math.max(...ranking.map(r => Math.abs(r.m.rentLiquidaMensal)), 0.01) }),
                React.createElement("table", { style: { width: '100%', borderCollapse: 'collapse', marginTop: 10 } },
                    React.createElement("thead", null,
                        React.createElement("tr", null,
